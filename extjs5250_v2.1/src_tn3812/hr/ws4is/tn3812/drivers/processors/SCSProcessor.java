...
class SCSProcessor extends SCSProcessorA {
	...
	public void process(ByteBuffer buffer){
	   ...
	   protected void processDATA(ByteBuffer textBuffer, byte current){
		   ...
		   if(listener!=null){
			   listener.onData(textBuffer, controls);
		   }
	   }
	   ...
	   protected void processFF(ByteBuffer buffer, ByteBuffer textBuffer){
		   ...
		   if(this.controls.currentFeedCounter == this.controls.SFFC){
			   ...
			   if(this.listener!=null){
				   textBuffer.flip();
				   listener.onNewLine(controls);
				   textBuffer.clear();
				   listener.onFormFeed(this.controls);
	   ...
	   protected void processNL(ByteBuffer buffer, ByteBuffer textBuffer){
		   ...
		   textBuffer.flip();
		   if(listener!=null){
			   listener.onNewLine(this.controls);
		   }
		   textBuffer.clear();
	   ...
	   protected void processPP(ByteBuffer buffer, ByteBuffer textBuffer){
		   ...
		   case SCSCodes.AMR : 
			   ...
			   break;
		   case SCSCodes.AMD :
			   controls.currentLine = controls.currentLine + value; 
			   //TODO ? 
			   break;
		   }
	   }
	   ...
	   protected void processBAC(ByteBuffer buffer, ByteBuffer textBuffer, short len){
		   ...
		   if(listener!=null){
			   listener.onFontStyleChange(controls, SCSStyleType.BOLDING);
		   }
	   }
	   ...
	   protected void processBES(ByteBuffer buffer, ByteBuffer textBuffer, short len){
		   ...
		   if(listener!=null){
			   listener.onFontStyleChange(controls, SCSStyleType.EMPHASIS);
		   }
	   }
	   ...
	   protected void processBUS(ByteBuffer buffer, ByteBuffer textBuffer, short len){
		   ...
		   if(listener!=null){
			   listener.onFontStyleChange(controls, SCSStyleType.UNDERSOCRE);
		   }
	   }
	   ...
	   protected void processBOS(ByteBuffer buffer, ByteBuffer textBuffer, short len){
		   ...
		   if(listener!=null){
			   listener.onFontStyleChange(controls, SCSStyleType.OVERSTRIKE);
		   }
	   ...
	   private ByteBuffer prepareData(ByteBuffer current, ByteBuffer previous){
			...
			if(previous!=null && previous.limit()>0){
				buffer = ByteBuffer.allocate(previous.remaining() + current.remaining());
				buffer.put(previous);
				buffer.put(current);
			} else {
				buffer = current;
